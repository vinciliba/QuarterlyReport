...
from reporting.quarterly_report.report_utils.enhanced_report_generator_old import (EnhancedReportGenerator,
...
class CommentsConfig:
    ...
    DEFAULT_MODEL = "codellama:13b"  # Use reasoning model by default
...
class CommentsModule(BaseModule):
    ...
    def run(self, ctx: RenderContext) -> RenderContext:
        ...
        try:
            # Get all configured sections directly
            sections_to_generate = CommentsConfig.SINGLE_SECTIONS.copy()
            
            print(f"📋 Generating ALL {len(sections_to_generate)} configured sections...")
            print(f"📋 Sections: {sections_to_generate}")
            
            # Get mapping for validation and later use
            mapping = TemplateSectionMatrix.get_complete_mapping_matrix()
            
            # Validate sections exist in mapping
            valid_sections = []
            for section in sections_to_generate:
                if section in mapping:
                    valid_sections.append(section)
                    print(f"   ✅ {section} - Found in mapping")
                else:
                    print(f"   ❌ {section} - NOT FOUND in mapping")
                    module_warnings.append(f"Section {section} not found in mapping matrix")
            
            sections_to_generate = valid_sections
            print(f"\n📊 Processing {len(sections_to_generate)} valid sections")
            
            # Add pre-generation diagnostics if enabled
            if CommentsConfig.GENERATION_CONTROL['enable_diagnostics']:
                print("\n🔍 PRE-GENERATION DIAGNOSTICS")
                print("="*50)
                for section in CommentsConfig.GENERATION_CONTROL['diagnostic_sections']:
                    if section in sections_to_generate:
                        diagnosis = generator.diagnose_section_generation(
                            section, quarter_period, current_year, financial_data, verbose=True
                        )
            ...
            for i, section_key in enumerate(sections_to_generate, 1):
                ...
                try:
                    ...
                    if section_key in ['heu_payment_overview', 'h2020_payment_overview']:
                        # Payment overview sections need special handling
                        print(f"🔄 Payment overview section - generating combinations...")
                        ...
                        for attempt in range(2):  # 2 attempts
                            ...
                            try:
                                ...
                                if commentary and len(commentary.strip()) > 50:
                                    # Fix any wrong time period references
                                    commentary = commentary.replace("Q2 2025", f"{quarter_period}")
                                    commentary = commentary.replace("Quarter 2", "Quarter 1")
                                    commentary = commentary.replace("second quarter", "first quarter")
                                    
                                    generation_success = True
                                    break
                                else:
                                    print(f"   ⚠️ Attempt {attempt + 1} produced insufficient content")
                                    
                            except Exception as e:
                                print(f"   ❌ Attempt {attempt + 1} error: {e}")
                                continue
                        
                        if generation_success and commentary:
                            ...
                            # Generation failed but continue
                            single_section_stats['failed'] += 1
                            error_msg = f"Failed to generate acceptable content for {section_key}"
                            module_warnings.append(error_msg)
                            print(f"⚠️ {error_msg} - CONTINUING WITH NEXT SECTION")
                ...
                # CRITICAL: Always continue to next section
            ...
            print(f"⏭️ Skipped: {single_section_stats['skipped']}")
            print(f"🤝 Human Validated: {single_section_stats['human_validated']}")
            print(f"💾 Variables Created: {len(single_section_stats['variables_created'])}")
            
            # Show what was created
            if single_section_stats['variables_created']:
                print(f"\n📋 Created variables:")
                for var in single_section_stats['variables_created']:
                    print(f"   • {var}")
            
            # Show generation timeline
            if generation_log:
                print(f"\n📅 GENERATION TIMELINE:")
                for entry in generation_log[-5:]:  # Show last 5 entries
                    if 'status' in entry:
                        status_icon = "✅" if entry.get('status') == 'success' else "❌"
                        duration = entry.get('duration', 0)
                        print(f"   {status_icon} {entry['section']}: {duration:.1f}s")
